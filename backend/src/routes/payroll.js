...
router.post('/batches', verifyToken, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      ...
      [b.id, b.company_id||b.companyId, b.period, b.month, b.working_days||b.workingDays||26,
...
router.put('/batches/:id/status', verifyToken, async (req, res) => {
  ...
  try {
    ...
    let q, params;
    if (note) {
      q = `UPDATE payroll_batches SET status=$2,${colMap[status]||''}, updated_at=NOW() WHERE id=$1 RETURNING *`;
      params = [req.params.id, status, actorName, ts, note].filter((_,i)=>i<5);
    } else {
      q = `UPDATE payroll_batches SET status=$2, updated_at=NOW() WHERE id=$1 RETURNING *`;
      params = [req.params.id, status];
    }
...
router.post('/entries', verifyToken, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      ...
      [e.batchId||e.batch_id, e.employeeId||e.employee_id, e.companyId||e.company_id||req.user.companyId,
